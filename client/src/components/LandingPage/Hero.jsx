...
import { ChevronDown, Users, Trophy, Timer, Star } from '../../icons/index.jsx';

const Hero = ({ scrollToSection }) => {
    const stats = [
        { value: '500+', label: 'Athletes', icon: <Users className="w-8 h-8" /> },
        { value: '25+', label: 'Events', icon: <Trophy className="w-8 h-8" /> },
    ...
    return (
        <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden">
            <div className="absolute inset-0 z-0">
                <div className="absolute inset-0 bg-linear-to-br from-blue-900/95 via-purple-900/90 to-cyan-900/95 z-10"></div>
            ...
            <div className="relative z-20 text-center px-4 max-w-6xl mx-auto pt-20">
                <div className="mb-8 animate-fade-in">
                    <span className="inline-block px-6 py-2 bg-white/10 backdrop-blur-md rounded-full text-cyan-300 font-semibold text-sm border border-cyan-400/30">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6 max-w-4xl mx-auto">
                    {stats.map((stat, idx) => (
                        <div key={idx} className="group relative p-4 sm:p-6 bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 hover:bg-white/20 transition-all duration-300 transform hover:-translate-y-2">
                            ...
                            <div className="text-xs sm:text-sm text-cyan-200">{stat.label}</div>
                        </div>
                    ))}
                </div>
            </div>
